...
@section('mainrep')
    ...
    <div class="h-screen flex items-center justify-center " >
        <p class="text-6xl  text-blue-700"> الجاليات الأجنبية </p>
    ...
    @php
        ...
        $familyshowAll=\App\Models\Familyshow::where('country_id','!=',1)->orderBy('country_id')->get();
    @endphp
    @foreach($familyshowAll as $show)
        @php
            $familyshow_id=$show->id;
        ...
        <div >
            ...
            <label  class="text-xl  font-bold text-red-500"> {{$familyshow->name}} </label>
            <label>&nbsp;&nbsp;&nbsp;&nbsp;</label>
            <label class="text-sm">عدد الضحايا  : </label>
            <label  class="text-sm text-red-600"> {{$count}} </label>
        </div>
            ...
            @if($fam->count()>1)
                @if(\App\Models\Victim::where('family_id',$family->id)->count()>0 )
                    <div style="position: relative;">
                        @if(!$loop->first) <br> @endif
                        <span style="font-size: 14pt;">
                            <label  style="color: #9f1239">{{$family->FamName}} :</label>
                            <br>
                         </span>
                    </div>
                @endif
                ...
                @foreach($victim_father->where('family_id',$family->id) as $victim)
                    <div  style="text-align: right;font-size: 11pt;" class="flex">
                        ...
                        @endif
                            ...
                            @php info($victim->FullName) @endphp
