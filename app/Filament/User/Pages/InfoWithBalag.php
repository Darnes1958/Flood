...
class InfoWithBalag extends Page implements HasTable,HasForms
{
      ...
      ->columns([
        TextColumn::make('FullName')
          ...
          ->description(function (Victim $record){
            ...
            if ($ded) {$slash=null; if ($ded->tel) $slash=' / ';
              $who=$who. "المبلغ ->   متوفيين : ".$ded->who.$slash.$ded->tel; if ($ded->ship) $who=$who.' ('.$ded->ship.') ';}
            if ($bal)
              if ($ded) {$slash=null; if ($bal->tel) $slash=' / ';
                $who=$who.'   بلاغات : '.$bal->who.$slash.$bal->tel;}
              else {$slash=null; if ($bal->tel) $slash=' / ';
                $who=$who.' المبلغ ->   بلاغات : '.$bal->who.$slash.$bal->tel;}
            if ($ded || $bal) {
              $who=$who.' الأم -> ';
              if ($ded && $ded->mother) $who=$who.'متوفيين : '.$ded->mother;
              if ($bal && $bal->mother) $who=$who.'بلاغات : '.$bal->mother;
            }
            return $who;
          })
