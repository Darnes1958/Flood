...
class ListDeads extends ListRecords
{
        ...
        return [
            ...
            Actions\Action::make('setfamily')
                ...
                ->action(function (){
                    ...
                    Dead::where('name','like','%'.'اسماعيل'.'%')->where('nation','ليبيا')->where('family_id',null)
                        ->update(['family_id'=>109]) ;
                    Dead::where('name','like','%'.'عزوز'.'%')->where('nation','ليبيا')->where('family_id',null)
                        ->update(['family_id'=>40]) ;
                    Dead::where('name','like','%'.'غفير'.'%')->where('nation','ليبيا')->where('family_id',null)
                        ->update(['family_id'=>132]) ;
                    Dead::where('name','like','%'.'بدر'.'%')->where('nation','ليبيا')->where('family_id',null)
                        ->update(['family_id'=>51]) ;
                    Dead::where('name','like','%'.'رافع'.'%')->where('nation','ليبيا')->where('family_id',null)
                        ->update(['family_id'=>184]) ;
                    Dead::where('name','like','%'.'نو'.'%')->where('nation','ليبيا')->where('family_id',null)
                        ->update(['family_id'=>130]) ;


                    Dead::where('family_id',null)
                        ->where('nation','ليبيا')
                        ->update(['family_id'=>45]) ;
                }),
            ...
            Actions\Action::make('Comparedead')
                ->label('مقارنة')
                ->icon('heroicon-m-users')
                ->color('info')
                ->url('deads/comparedead'),
        ];
    }
}
