...
class DeadWidget extends BaseWidget
{
    public $family_id;
    public $repeted;
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->striped()
            ->defaultSort('name')
            ->heading(new HtmlString('<div class="text-primary-400 text-lg">متوفيين</div>'))
            ->columns([
                TextColumn::make('name')
                    ->description(function (Dead $record){
                        if ($this->show_description)
                            if ($record->mother) return $record->mother;
                    })
                    ->action(function (Dead $record){
                ...
                TextColumn::make('Victim.FullName')
                    ...
                    ->searchable()
                    ->action(function (Dead $record){
                        Victim::find($record->victim_id)->update(['dead'=>null]);
                ...
                Tables\Columns\IconColumn::make('repeted')
                    ->label('مكرر')
                    ->action(function (Dead $record): void {
